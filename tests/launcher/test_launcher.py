...
from launcher.config import parse_args, determine_proxy_configuration, LauncherConfig
from launcher.utils import is_port_in_use, find_pids_on_port, kill_process_interactive, get_proxy_from_gsettings
from launcher.checks import ensure_auth_dirs_exist, check_dependencies
...
def test_parse_args_defaults():
    ...
    with patch.dict(os.environ, {'STREAM_PORT': '3120'}, clear=True), \
...
def test_determine_proxy_configuration_arg():
    result = determine_proxy_configuration(internal_camoufox_proxy_arg="http://proxy:8080")
...
def test_kill_process_interactive_linux(mock_system, mock_run):
    ...
    mock_run.assert_called_with("kill 1234", shell=True, capture_output=True, text=True, timeout=3, check=False)
...
def test_kill_process_interactive_windows(mock_system, mock_run):
    mock_system.return_value = "Windows"
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = "SUCCESS: The process with PID 1234 has been terminated."
