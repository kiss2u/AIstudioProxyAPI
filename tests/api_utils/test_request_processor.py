...
async def test_process_request_success_stream(mock_env, mock_request_context):
    ...
    with (
        ...
        patch(
            "api_utils.clear_stream_queue", new_callable=AsyncMock
        ) as mock_clear_queue,
    ):
        mock_conn.return_value = True
        mock_init_ctx.return_value = mock_request_context
...
@pytest.mark.asyncio
async def test_handle_auxiliary_stream_response_non_stream_success(
    mock_env, mock_request_context
):
    req_id = "test-req-id"
    request_data = ChatCompletionRequest(
        messages=[Message(role="user", content="Hello")],
...
async def test_handle_model_switch_failure():
    ...
    state.current_ai_studio_model_id = "changed"
    ...
    try:
        ...
        with pytest.raises(HTTPException) as exc:
            await _handle_model_switch_failure(
                req_id, page, "new-model", "old-model", logger
            )

        assert exc.value.status_code == 422
        assert state.current_ai_studio_model_id == "old-model"
    finally:
