...
async def test_gen_sse_from_aux_stream_error_in_processing(
    ...
    """Test error chunk generation when exception occurs during processing (lines 215-239)."""
    ...
    # Should have error chunk with Chinese error message
    ...
    for chunk in chunks:
        ...
        try:
            ...
            if "错误" in content and "Simulated stream processing error" in content:
                error_chunk_found = True
                assert data["choices"][0]["finish_reason"] == "stop"
        except (json.JSONDecodeError, KeyError):
            continue

    assert error_chunk_found, "Should generate error chunk"
...
async def test_gen_sse_from_playwright_exception_in_error_handling(
    ...
    """Test exception during error chunk generation (lines 351-354)."""
    ...
    with (
        patch("browser_utils.page_controller.PageController") as MockPC,
        patch(
            "api_utils.response_generators.generate_sse_chunk",
            side_effect=RuntimeError("Chunk gen failed"),
        ),
    ):
        instance = MockPC.return_value
        ...
        chunks = []
        # Should not raise, should silently handle error
        async for chunk in gen_sse_from_playwright(
            mock_page,
            mock_logger,
            req_id,
