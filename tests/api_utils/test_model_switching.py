...
mock_server.current_ai_studio_model_id = "gemini-1.5-pro"
...
async def test_analyze_model_requirements_no_requested_model(base_context):
    ...
    result = await analyze_model_requirements(req_id, base_context, requested_model, proxy_model_name)
...
async def test_analyze_model_requirements_invalid_model(base_context):
    ...
    with pytest.raises(HTTPException) as exc:
        await analyze_model_requirements(req_id, base_context, requested_model, proxy_model_name)
...
@pytest.mark.asyncio
async def test_handle_model_switching_success(base_context):
    req_id = "test-req"
    base_context["needs_model_switching"] = True
    base_context["model_id_to_use"] = "gemini-1.5-flash"
    mock_server.current_ai_studio_model_id = "gemini-1.5-pro"
    
    with patch("browser_utils.switch_ai_studio_model", new_callable=AsyncMock) as mock_switch:
        mock_switch.return_value = True
        
        result = await handle_model_switching(req_id, base_context)
        
        mock_switch.assert_called_once_with(base_context["page"], "gemini-1.5-flash", req_id)
        assert mock_server.current_ai_studio_model_id == "gemini-1.5-flash"
        assert result["model_actually_switched"] is True
        assert result["current_ai_studio_model_id"] == "gemini-1.5-flash"

@pytest.mark.asyncio
async def test_handle_model_switching_failure(base_context):
    req_id = "test-req"
    base_context["needs_model_switching"] = True
    base_context["model_id_to_use"] = "gemini-1.5-flash"
    mock_server.current_ai_studio_model_id = "gemini-1.5-pro"
    ...
    with patch("browser_utils.switch_ai_studio_model", new_callable=AsyncMock) as mock_switch:
        ...
        assert mock_server.current_ai_studio_model_id == "gemini-1.5-pro"  # Should revert/stay same

@pytest.mark.asyncio
async def test_handle_model_switching_already_switched(base_context):
    req_id = "test-req"
    base_context["needs_model_switching"] = True
    base_context["model_id_to_use"] = "gemini-1.5-flash"
    mock_server.current_ai_studio_model_id = "gemini-1.5-flash"  # Already matches
...
async def test_handle_parameter_cache_switched(base_context):
    ...
    base_context["page_params_cache"] = {"some": "cache", "last_known_model_id_for_params": "gemini-1.5-pro"}
    
    await handle_parameter_cache(req_id, base_context)
    
    assert base_context["page_params_cache"] == {"last_known_model_id_for_params": "gemini-1.5-flash"}
...
async def test_handle_parameter_cache_valid(base_context):
    ...
    assert base_context["page_params_cache"]["last_known_model_id_for_params"] == "gemini-1.5-pro"