"""
Consolidated tests for model_management edge cases, error handling, and revert logic.

Merged from:
- test_model_management_coverage.py
- test_model_management_coverage_part2.py
- test_model_management_coverage_part3.py
- test_model_management_extended.py (original)
- test_model_management_revert.py
"""
...
@pytest.mark.asyncio
async def test_force_ui_state_retry_failure(mock_page):
    """Test retry logic failing all attempts."""
    with patch("browser_utils.model_management._force_ui_state_settings") as mock_force:
...
@pytest.mark.asyncio
@pytest.mark.timeout(5)
async def test_set_model_from_page_display_timeout(mock_page, mock_server):
    """Test timeout waiting for model list."""
    mock_server.model_list_fetch_event.is_set.return_value = False
    mock_server.model_list_fetch_event.wait.side_effect = asyncio.TimeoutError
    ...
    with (
        ...
        warnings = [call.args[0] for call in mock_logger.warning.call_args_list]
        assert any("等待模型列表超时" in w for w in warnings)
