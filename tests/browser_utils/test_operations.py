...
@patch('builtins.open')
def test_get_injected_models(mock_open, mock_exists, mock_env):
...
async def test_handle_model_list_response_success():
    ...
    with patch('server.parsed_model_list', []), \
...
async def test_get_response_via_edit_button_success(mock_page):
    ...
    with patch('playwright.async_api.expect', new_callable=MagicMock) as mock_expect:
        ...
        mock_page.locator.return_value.last.get_by_label = MagicMock(return_value=edit_btn)
        ...
        # No, await (expect_async(...)).to_be_visible(...)
        # If expect_async(...) returns a coroutine, then (await expect_async(...)).to_be_visible(...) would be valid if we awaited it.
        # But the code is await expect_async(edit_button).to_be_visible(...)
        # This parses as await (expect_async(edit_button).to_be_visible(...))
        ...
        result = await get_response_via_edit_button(mock_page, "req_id", check_disconnect)
...
async def test_get_response_via_copy_button_success(mock_page):
    ...
    with patch('playwright.async_api.expect', new_callable=MagicMock) as mock_expect:
        ...
        result = await get_response_via_copy_button(mock_page, "req_id", check_disconnect)
...
async def test_wait_for_response_completion_success(mock_page):
    ...
    result = await _wait_for_response_completion(
        mock_page, prompt_area, submit_btn, edit_btn,
        "req_id", check_disconnect, "chat_id",
        timeout_ms=1000, initial_wait_ms=0
    )
...
async def test_get_final_response_content_edit_success(mock_page):
    ...
    with patch('browser_utils.operations_modules.interactions.get_response_via_edit_button', new_callable=AsyncMock) as mock_edit:
        mock_edit.return_value = "Content"
        
        result = await _get_final_response_content(mock_page, "req_id", check_disconnect)
...
async def test_get_final_response_content_fallback_copy(mock_page):
    ...
    with patch('browser_utils.operations_modules.interactions.get_response_via_edit_button', new_callable=AsyncMock) as mock_edit, \
         patch('browser_utils.operations_modules.interactions.get_response_via_copy_button', new_callable=AsyncMock) as mock_copy:
